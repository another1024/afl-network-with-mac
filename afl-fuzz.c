...
int network_listen(void) {
  ...
  if (N_rp->ai_socktype == SOCK_STREAM) {
    ...
    #if defined(__APPLE__) || defined(__FreeBSD__) || defined (__OpenBSD__)
          client_fd = accept(N_fd, (struct sockaddr *) &N_myaddr,
                  &N_myaddrlen);
          fcntl(client_fd, O_CLOEXEC);
    ...
    #if defined(__APPLE__) || defined(__FreeBSD__) || defined (__OpenBSD__)
        if (sendfile(fd, client_fd, 0, &statbuf.st_size,NULL,0) == -1) {
  ...
  if (N_results != NULL) {
    ...
    if (N_results->ai_socktype == SOCK_STREAM) {
...

      {
        ...
        /* use sendfile() to transfer the data if possible because it is efficient */
        #if defined(__APPLE__) || defined(__FreeBSD__) || defined (__OpenBSD__)
            if (sendfile(fd, N_fd, 0, &statbuf.st_size,NULL,0) == -1) {
